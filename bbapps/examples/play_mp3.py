...
# ///
"""Play an mp3 file on speaker.audio. usage: play_mp3.py [file.mp3]"""
import sys
...
VOLUME = 0.3  # 1.0 = original level, 0.5 = half, etc.
...
def main():
    path = resolve(sys.argv[1]) if len(sys.argv) > 1 else find_default_mp3()
    print(f"playing {path.name}", flush=True)
    with Writer("speaker.audio", Type("speaker_audio")) as w:
        for chunk in load(path):
            with w.buf() as b:
                b["audio"] = chunk
